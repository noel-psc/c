/**
 * 1、小容量可以自动赋值给大容量，称为自动类型转换。
 * 2、大容量不能直接赋值给小容量，需要使用强制类型转换符进行强转。
 * 	但需要注意的是：加强制类型转换符之后，虽然编译通过了，但是
 * 	运行的时候可能会损失精度。
 */
public class IntTest03 {
	public static void main(String[] args) {
		...
		//以上的long类型100L强转int类型，会自动将“前面”的4个字接砍掉：00000000 00000000 00000000 01100100
		int y = (int)x; //这个(int)就是强制类型转换符，加上去就能编译通过。
				  //但是要记住，虽然编译通过了，但是运行时可能损失精度。
